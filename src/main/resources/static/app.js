  ...
  $.getJSON("/timeTable", function (timeTable) {
      ...
      rowByTeacher
        .append($(`<th class="align-middle"/>`)
          .append($("<span/>").text(`
                ...
                `)));
      $.each(teacherList, (index, teacher) => {
        rowByTeacher.append($("<td/>").prop("id", `timeslot${timeslot.id}teacher${teacher.id}`));
      });
      ...
      rowByStudentGroup
        .append($(`<th class="align-middle"/>`)
          .append($("<span/>").text(`
                ...
                `)));
      $.each(studentGroupList, (index, studentGroup) => {
        rowByStudentGroup.append($("<td/>").prop("id", `timeslot${timeslot.id}studentGroup${studentGroup.id}`));
      });
    });
    ...
    $.each(timeTable.lessonList, (index, lesson) => {
      ...
      if (lesson.timeslot == null || lesson.room == null) {
        ...
        $(`#timeslot${lesson.timeslot.id}teacher${lesson.teacher.id}`).append(lessonElementWithoutDelete.clone());
        $(`#timeslot${lesson.timeslot.id}studentGroup${lesson.studentGroup.id}`).append(lessonElementWithoutDelete.clone());
      }
    });
  });
}
